...
async function popularFiltroMontadores() {
    ...
    if (!select) return;
    try {
        const res = await fetch(`${API_BASE_URL}/api/montadores`, { headers: { 'Authorization': `Bearer ${localStorage.getItem("authToken")}` } });
        const data = await res.json();
        select.innerHTML = '<option value="">Todos os Montadores</option>';
        data.forEach(m => select.innerHTML += `<option value="${m.id}">${m.nome}</option>`);
...
async function handleGerarRelatorio(e) {
    ...
    try {
        ...
        filtros.reportType === 'montador' ? renderizarTabelaMontadores(data.tableData) : renderizarTabelaProjetos(data.tableData);
...
function renderKPIs(stats, type) {
    ...
    if (type === 'projeto') {
        row.innerHTML = 
            card('Concluídos', `${stats.percConcluidos}%`, 'fa-check-circle', 'text-success') +
            card('Em Montagem', `${stats.percEmMontagem}%`, 'fa-cogs', 'text-primary') +
            card('Pendentes', `${stats.percPendentes}%`, 'fa-hourglass-half', 'text-warning') +
            card('Taxa de N/C', `${stats.percNaoConformes}%`, 'fa-exclamation-triangle', 'text-danger');
    } else {
        row.innerHTML = 
            ...
            card('Taxa N/C Geral', `${stats.taxaNaoConformidadeGeral}%`, 'fa-exclamation-triangle', 'text-danger');
    }
}
...
function renderizarGrafico(chartData) {
    ...
    let config = {
        ...
        options: {
            ...
            plugins: { legend: { position: chartData.type === 'pie' ? 'top' : 'bottom' } }
        }
    };
    ...
    if (chartData.type === 'bar') {
        ...
        config.options.scales = {
            x: { beginAtZero: true, ticks: { color: '#bbb', precision:0 }, grid: { color: 'rgba(255,255,255,0.1)', borderDash:[2,4] } },
...
function renderizarTabelaProjetos(data) {
    ...
    data.forEach(i => {
        ...
        const nc = i.teveNaoConformidade ? ' <i class="fas fa-exclamation-circle text-danger" title="N/C"></i>' : '';
...
async function exportarPDF() {
    ...
    if (currentReportChart) { Chart.defaults.color = '#000'; currentReportChart.update('none'); }
